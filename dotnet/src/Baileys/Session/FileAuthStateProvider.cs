...
public sealed class FileAuthStateProvider : IAuthStateProvider
{
	...
	/// <inheritdoc/>
	public async Task<AuthenticationCreds> LoadCredsAsync(
	...
	)
	{
		...
		try
		{
			...
			var dto = await JsonSerializer
				.DeserializeAsync<AuthCredsDto>(stream, JsonOptions, cancellationToken)
	...
	/// <inheritdoc/>
	public async Task SaveCredsAsync(
	...
	private sealed class AuthCredsDto
	{
		...
		public AuthenticationCreds ToAuthenticationCreds() =>
			new()
			{
				...
				SignedPreKey =
					SignedPreKey?.ToSignedKeyPair() ?? new SignedKeyPair(new KeyPair([], []), [], 0),
		...
		public static AuthCredsDto FromAuthenticationCreds(AuthenticationCreds c) =>
			new()
			{
	...
	private sealed class KeyPairDto
	{
		...
		public static KeyPairDto FromKeyPair(KeyPair kp) =>
			new()
			{
	...
	private sealed class SignedKeyPairDto
	{
		...
		public SignedKeyPair ToSignedKeyPair() =>
			new(
		...
		public static SignedKeyPairDto FromSignedKeyPair(SignedKeyPair skp) =>
			new()
			{
	...
	private sealed class AccountSettingsDto
	{
		...
		public static AccountSettingsDto FromAccountSettings(AccountSettings s) =>
			new()
			{
